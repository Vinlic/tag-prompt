...
export default class Compiler {
    ...
    compile(value: any, scope: any = {}): any {
        if (util.isString(value)) {  //字符串处理
            ...
            const result: any = {};
            if (attrs.type === "component")  //组件作用域生成
                scope = this.generateComponentScope(attrs, scope);
    ...
    generateComponentScope(attrs, scope) {
        let config, data;
        const _scope = { ...scope };
        const configExpressions = this.extractExpressions(attrs.config);
        if (configExpressions.length) {
            const { expression } = configExpressions[0];
            attrs.config = this.eval(expression, scope) as any;
        }
        if (util.isString(attrs.config)) {
            let raw;
            try {
                raw = JSON.parse(attrs.config);
            }
            catch (err) {
                throw new Error("config string must be json format");
            }
            config = this.compile(raw, _scope);
            config && Object.assign(_scope, config, this.dataset);
        }
        else if (util.isObject(attrs.config)) {
            config = this.compile(attrs.config, _scope);
            config && Object.assign(_scope, config, this.dataset);
        }
        else if (util.isString(attrs.configSrc)) {
            config = util.decodeSrc(attrs.configSrc);
            config && Object.assign(_scope, config, this.dataset);
        }
        const dataExpressions = this.extractExpressions(attrs.data);
        if (dataExpressions.length) {
            const { expression } = dataExpressions[0];
            attrs.data = this.eval(expression, scope) as any;
        }
        if (util.isString(attrs.data)) {
            let raw;
            try {
                raw = JSON.parse(attrs.data);
            }
            catch (err) {
                throw new Error("data string must be json format");
            }
            data = this.compile(raw, _scope);
            data && Object.assign(_scope, data, this.dataset);
        }
        else if (util.isObject(attrs.data)) {
            data = this.compile(attrs.data, _scope);
            data && Object.assign(_scope, data, this.dataset);
        }
        else if (util.isString(attrs.dataSrc)) {
            data = util.decodeSrc(attrs.dataSrc);
            data && Object.assign(_scope, data, this.dataset);
        }
        return _scope;
    }
    ...
    extractExpressions(value: any) {
        ...
        let match
        try {
            match = value.toString().match(/(?<={{).*?(?=}})/gs); //匹配所有的表达式
        }
    ...
    generateGlobal(script: string, extendFunctions: any) {
        ...
        const scope = { ...this.dataset, ...functions, ...extendFunctions, dayjs };
        ...
        if (util.isObject(scriptFunctions)) {
            for (let name in scriptFunctions) {
                if (!util.isFunction(scriptFunctions[name]))
                    continue;
                scriptFunctions[name] = scriptFunctions[name].bind(scriptFunctions);
            }  //this指向修改
        }
        return {
            ...scope,
